...
def main(args=None):
    ...
    weather_config = yaml.safe_load(open(config_file))['weather']
    ...
    try:
        ...
        weather_cfg = carla_world.get_weather()
        print(weather_cfg)
        # time.sleep(15)
        weather_cfg.cloudiness = weather_config["cloudiness"]
        weather_cfg.precipitation = weather_config["precipitation"]
        weather_cfg.sun_azimuth_angle = weather_config["sun_azimuth_angle"]
        weather_cfg.fog_density = weather_config["fog_density"]
        weather_cfg.precipitation_deposits = weather_config['precipitation_deposits']
        weather_cfg.sun_altitude_angle = weather_config['sun_altitude_angle']
        weather_cfg.wind_intensity = weather_config['wind_intensity']
        # weather_cfg.fog_distance = weather_config['fog_distance']
        weather_cfg.wetness = weather_config['wetness']
        weather_cfg.dust_storm = weather_config['dust_storm']
        # weather_cfg.scattering_intensity = weather_config['scattering_intensity']
        # weather_cfg.mie_scattering_scale = weather_config['mie_scattering_scale']
        # weather_cfg.rayleigh_scattering_scale = weather_config['rayleigh_scattering_scale']


        carla_world.set_weather(weather_cfg)
