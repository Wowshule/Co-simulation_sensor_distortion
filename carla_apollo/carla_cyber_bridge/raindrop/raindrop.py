...
class raindrop():
	...
	def updateTexture_GaussianFilter(self, bg):
		fg = pyblur.GaussianBlur(Image.fromarray(np.uint8(bg)), 15)
		fg = np.asarray(fg)

		# Convert to RGBA format by adding an alpha channel
		tmp = np.expand_dims(self.alphamap, axis=-1)    
		tmp = np.concatenate((fg, tmp), axis=2)

		# Create a texture image from the numpy array
		self.texture = Image.fromarray(tmp.astype('uint8'), 'RGBA')
	...
	def _create_label(self):
		...
		elif self.type == "Mosaic":
			self._createMosaicDrop()
	...
	def _createMosaicDrop(self):
		cv2.circle(self.labelmap, (self.radius * 2, self.radius * 3), self.radius, 255, -1)
		cv2.ellipse(self.labelmap, (self.radius * 2, self.radius * 3), (self.radius, int(1.3*math.sqrt(3) * self.radius)), 0, 180, 360, 255, -1)		
		# set alpha map for png 
		self.alphamap = pyblur.GaussianBlur(Image.fromarray(np.uint8(self.labelmap)), 10)
		self.alphamap = np.asarray(self.alphamap).astype(np.float)
		self.alphamap = self.alphamap/np.max(self.alphamap)*255.0
